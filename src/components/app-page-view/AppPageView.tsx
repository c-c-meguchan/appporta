...
export function AppPageView({ data: d, reviews = [], preview, onSectionFocus, focusedSectionId, onSectionHover, hoveredSectionId, appID }: AppPageViewProps) {
  ...
  // ヒーローがスクロールで見えなくなったらスティッキーナビを表示
  useEffect(() => {
    const heroEl = heroSectionRef.current;
    if (!heroEl) return;
    let scrollRoot: Element | null = null;
    for (let el: Element | null = heroEl.parentElement; el; el = el.parentElement) {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        const e = entries[0];
        if (e) setShowStickyNav(!e.isIntersecting);
      },
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50">
      {/* ヒーローが画面外に出たときだけ表示（DOMに出すので上に隙間ができない） */}
      {showStickyNav && (
        <header className="sticky top-0 z-10 border-b-[0.7px] border-zinc-200 bg-white/95 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
          <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
            ...
            {d.primary_link && (
              <a
                ...
                className="shrink-0 rounded-lg bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200"
              >
      ...
      <main className="mx-auto max-w-3xl px-4 pt-4 pb-8">
        {/* Hero Header */}
        <SectionWrapper
